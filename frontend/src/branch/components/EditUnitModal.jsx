...
import { useUpdateUnitName } from "../../branch/hooks/useUpdateUnitName";
import { useBranchesStore } from "../store/useBranchesStore";

export const EditUnitModal = ({ branchId, unitId, unitName, onClose }) => {
  const [name, setName] = useState(unitName || "");
  const [error, setError] = useState("");

  const updateUnitMutation = useUpdateUnitName();
  const { updateUnit } = useBranchesStore();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      updateUnit(branchId, unitId, { name: newName });

      onClose();
    } catch (err) {
      console.error("Error updating unit:", err);
      setError("خطا در ویرایش واحد رخ داد.");
    }
  };
  ...
  const modal = (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-3 bg-black/40 dark:bg-black/50 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-lg overflow-hidden shadow-xl bg-[#F4F4F5] dark:bg-[#0D1525] border border-gray-300 dark:border-gray-700">
        <ModalHeader title="ویرایش واحد" onClose={onClose} />
        ...
        <div className="flex items-center justify-end gap-3 p-4 bg-[#F4F4F5] dark:bg-[#0D1525]">
          <button
            ...
            disabled={updateUnitMutation.isLoading}
          ...
          >
            {updateUnitMutation.isLoading ? "در حال ذخیره..." : "ذخیره تغییرات"}
          </button>
          <button
            onClick={onClose}
