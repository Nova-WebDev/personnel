...
class UpdateUser:
    ...
    async def execute(
        ...
        user, previous_photo_path = await self.user_repository.update(
            user_id=user_id,
            phone=phone,
        ...
        if file_bytes is not None:
            ...
            if previous_photo_path is None:
                photo_path = f"{PHOTO_FOLDER}/{user_id}"
                await self.user_repository.set_photo_path(user_id, photo_path)
                user.photo_path = photo_path

        member_targets = await self.permission_repository.get_global_and_unit_scoped_user_ids(unit_id)
        targets = list(set(member_targets) | {user_id})
